...
#  systemd.timers."my-daily-script" = {
#    wantedBy = [ "timers.target" ];
#    timerConfig = {
#      OnCalendar = "daily";
#      Persistent = true;
#      Unit = "keepassbackup.service";
#    };
#  };
#  
#  systemd.services."keepassbackup" = {
#    script = ''
#      
#    '';
#    serviceConfig = {
#      Type = "oneshot";
#      User = "root";  # You can change the user if needed
#    };
#  };
...
#      shellInit 
	...
	ll = "ls -l";
	ls = "ls --color=tty";
        conv = "sudo code ~/projects/nix/configuration.nix";
	revuild = "sudo cp ~/projects/nix/configuration.nix /etc/nixos/configuration.nix && sudo nixos-rebuild switch && [ $? -eq 0 ] && now=$(date +'%Y-%m-%d_%H-%M-%S') && sudo cp /etc/nixos/configuration.nix /etc/nixos/conffiles/backups/configuration.$now.nix";
