  ...
  programs = {
    bash = {
      ...
      shellAliases = {
        ...
        revuild = "sudo cp ~/projects/nix/configuration.nix /etc/nixos/configuration.nix && sudo nixos-rebuild switch && [ $? -eq 0 ] && git -C ~/projects/nix add . && git -C ~/projects/nix commit -m '$(date) auto rebuild commit' && git -C ~/projects/nix push";
