...

{
  programs.bash = {
    shellAliases = {
      ...
      reebuild = "sudo nixos-rebuild switch --flake ~/projects/nix#nixos && git -C ~/projects/nix add . && git -C ~/projects/nix commit -m 'Auto-update flake & rebuild' && git -C ~/projects/nix push";
